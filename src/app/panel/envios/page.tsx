      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-[#8899aa] mt-2">
            Configura las tarifas de envío por departamento. {activeCount} de {rates.length} departamentos activos.
          </p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-[#1a2535] border border-[#2a3a4a] rounded-xl p-4">
            <p className="text-[#667788] text-xs uppercase tracking-wider">Departamentos</p>
            <p className="text-2xl font-bold text-white mt-1">{rates.length}</p>
          </div>
          <div className="bg-[#1a2535] border border-[#2a3a4a] rounded-xl p-4">
            <p className="text-[#667788] text-xs uppercase tracking-wider">Tarifa más baja</p>
            <p className="text-2xl font-bold text-green-400 mt-1">{formatPrice(Math.min(...rates.map((r) => r.rate)))}</p>
          </div>
          <div className="bg-[#1a2535] border border-[#2a3a4a] rounded-xl p-4">
            <p className="text-[#667788] text-xs uppercase tracking-wider">Tarifa más alta</p>
            <p className="text-2xl font-bold text-orange-400 mt-1">{formatPrice(Math.max(...rates.map((r) => r.rate)))}</p>
          </div>
        </div>

        {/* Table */}
        <div className="bg-[#1a2535] border border-[#2a3a4a] rounded-xl overflow-hidden">
          <div className="grid grid-cols-12 gap-2 px-5 py-3 border-b border-[#2a3a4a] text-[#667788] text-xs uppercase tracking-wider font-medium">
          ...
          <div className="divide-y divide-[#2a3a4a]">
            {rates.map((rate) => {
              ...
              return (
                <div key={rate.department} className="grid grid-cols-12 gap-2 px-5 py-3 items-center hover:bg-[#1e2d40] transition-colors">
                  ...
                  <div className="col-span-3">
                    {isEditing ? (
                      <input
                        ...
                        className="w-full bg-[#0f1923] border border-[#2a3a4a] rounded px-2 py-1 text-sm text-white focus:border-primary focus:outline-none"
                      />
                    ) : (
                  ...
                  <div className="col-span-3">
                    {isEditing ? (
                      ...
                      <span className="text-[#8899aa] text-sm">{rate.estimatedDays}</span>
                    )}
                  </div>
                  ...
                  <div className="col-span-1 flex justify-center">
                    {isEditing ? (
                      ...
                      <button onClick={() => startEdit(rate)} className="text-[#667788] hover:text-primary text-xs transition">
        ...
        {/* Help */}
        <div className="mt-6 bg-[#1a2535] border border-[#2a3a4a] rounded-xl p-5">
